...
@RestController @RequestMapping("/api/v1/picking") @RequiredArgsConstructor public class PickingController {
 private final PickingService service; private final com.myeshopping.pickingservice.service.PickingFulfillmentService fulfillmentService; @PostMapping("/orders/{orderId}/fulfill") public ResponseEntity<ApiResponse<PickingTask>> fulfill(@PathVariable Long orderId,@RequestParam(defaultValue="1") int itemCount){return ok("Order picked successfully",fulfillmentService.fulfill(orderId,itemCount));}
